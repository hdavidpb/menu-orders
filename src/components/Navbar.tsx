...
export const Navbar = () => {
  ...
  const totalProductsInCard = Object.values(state.productsInCart).reduce((acc, value) => {
    acc += value.quantity;
    return acc;
  }, 0);
  ...
  return (
    <nav className="w-full flex justify-between items-center p-4">
      <h1 className="text-4xl">Menú</h1>
      <Link href="/orders" className="bg-indigo-500 text-white p-2 rounded-lg flex justify-center items-center gap-1 transition-all ease-out hover:opacity-70 hover:shadow-lg">
        <span className="font-bold"> {totalProductsInCard}</span>
