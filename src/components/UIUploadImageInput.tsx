...
export const UIUploadImageInput = ({handleChangeFile,isLoading,fileImage,label,error}:Props) => {
  return (
      ...
      <div className={`w-full h-[150px] flex flex-col justify-center items-center rounded-lg border relative border-dashed cursor-pointer ${error?"border-red-500":""}`}>
          <input
            ...
            {fileImage && (<Image width={400} height={400} className="w-full h-full object-contain" alt="imagen de producto" src={URL.createObjectURL(fileImage)}/> )}
          </div>
